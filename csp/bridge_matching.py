...
def make_bridge_matching_loss_fn(
    ...
    *,
...
) -> Callable[[ConditionalDriftNet, jax.Array], jax.Array]:
    """Return a pure loss function for sequential conditional Brownian bridge matching.

    Args:
        latent_data: Stored paired trajectories with shape (T, N, K), ordered
            fine-to-coarse so latent_data[0] is the finest level and
            latent_data[-1] is the coarse conditioning level.
        zt: Stored data-time grid aligned with latent_data, strictly increasing
            from fine to coarse.
    """
    ...
    def loss_fn(params: ConditionalDriftNet, key: jax.Array) -> jax.Array:
        drift = eqx.combine(params, static_model)
        batch_key, interval_key, time_key, bridge_key = jax.random.split(key, 4)
        batch = _select_batch(latent_canonical, batch_size, batch_key)
        global_condition = batch[0]
        interval_idx = jax.random.randint(interval_key, (), 0, num_intervals)
        ...
        x_t = sample_brownian_bridge(x_start, x_end, t, t_start, t_end, sigma, bridge_key)
