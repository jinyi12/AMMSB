...
DEFAULT_THETA_FEATURE_CLIP = 1e-4
PAIRED_PRIOR_BRIDGE_MODEL_TYPE = "paired_prior_bridge"
PAIRED_PRIOR_BRIDGE_TRAINING_OBJECTIVE = "paired_prior_bridge_state_prediction"

PairedPriorDriftNet = ConditionalDriftNet | ConditionalTransformerDriftNet
def _validate_bridge_hyperparameters(delta_v: float, theta_trim: float, theta_feature_clip: float) -> None:
    if float(delta_v) <= 0.0:
        raise ValueError(f"delta_v must be > 0, got {delta_v}.")
    if not 0.0 <= float(theta_trim) < 0.5:
        raise ValueError(f"theta_trim must lie in [0, 0.5), got {theta_trim}.")
    if not 0.0 <= float(theta_feature_clip) < 0.5:
        raise ValueError(f"theta_feature_clip must lie in [0, 0.5), got {theta_feature_clip}.")
def sample_ve_paired_interval(
    x_prev: jax.Array,
    x_next: jax.Array,
    ...
    """Sample the VE/Brownian local bridge state with shared interval variance increment."""
    x_prev_arr = jnp.asarray(x_prev)
    x_next_arr = jnp.asarray(x_next, dtype=x_prev_arr.dtype)
    theta_arr = jnp.asarray(theta, dtype=x_prev_arr.dtype)
    delta_v_arr = jnp.asarray(delta_v, dtype=x_prev_arr.dtype)

    mean = (1.0 - theta_arr) * x_prev_arr + theta_arr * x_next_arr
    variance = jnp.maximum(theta_arr * (1.0 - theta_arr) * delta_v_arr, 0.0)
    noise = jax.random.normal(key, x_prev_arr.shape, dtype=x_prev_arr.dtype)
...
def bridge_logsnr(
    ...
    delta_v: float,
    *,
    theta_clip: float = 0.0,
) -> jax.Array:
    """Return lambda_br = log(theta / ((1 - theta) * DeltaV))."""
    theta_arr = jnp.asarray(theta, dtype=jnp.float32)
    clip_value = max(float(theta_clip), 1e-12)
    theta_safe = jnp.clip(theta_arr, clip_value, 1.0 - clip_value)
    delta_v_arr = jnp.asarray(delta_v, dtype=theta_safe.dtype)
    return jnp.log(theta_safe / ((1.0 - theta_safe) * delta_v_arr))


def clip_bridge_logsnr_to_prior_support(
    logsnr: jax.Array | float,
    prior_logsnr_max: float,
) -> jax.Array:
    bound = abs(float(prior_logsnr_max))
    return jnp.clip(jnp.asarray(logsnr, dtype=jnp.float32), -bound, bound)


def matched_prior_time_from_bridge_logsnr(
    logsnr: jax.Array | float,
    prior_logsnr_max: float,
) -> jax.Array:
    """Map bridge log-SNR to the prior's time coordinate after support clipping."""
    clipped = clip_bridge_logsnr_to_prior_support(logsnr, prior_logsnr_max)
    return jax.nn.sigmoid(-0.5 * clipped)


def matched_prior_time_from_theta(
    theta: jax.Array | float,
    delta_v: float,
    prior_logsnr_max: float,
    *,
    theta_clip: float = DEFAULT_THETA_FEATURE_CLIP,
) -> jax.Array:
    return matched_prior_time_from_bridge_logsnr(
        bridge_logsnr(theta, delta_v, theta_clip=theta_clip),
        prior_logsnr_max,
    )


def resolve_prior_logsnr_max_from_checkpoint_payload(ckpt: dict) -> float:
    ckpt_args = ckpt.get("args", {}) if isinstance(ckpt, dict) else {}
    ckpt_arch = ckpt.get("architecture", {}) if isinstance(ckpt, dict) else {}
    ...
    uses_prior = bool(
        ...
        or str(ckpt_args.get("loss_type", "")).lower() == "ntk_prior_balanced"
    )
    if not uses_prior:
        raise ValueError(
            "The supplied FAE checkpoint does not appear to be prior-trained. "
            "This paired-prior CSP path only supports checkpoints with an active diffusion prior."
        )
    ...
    raise ValueError("Prior-trained FAE checkpoint is missing prior_logsnr_max in args/architecture metadata.")


def resolve_prior_logsnr_max_from_checkpoint_path(path: str | Path) -> float:
    checkpoint_path = Path(path).expanduser().resolve()
    return resolve_prior_logsnr_max_from_checkpoint_payload(load_fae_checkpoint(checkpoint_path))
...
def _sample_theta(
    *,
    ...
    u = jax.random.uniform(key, (int(batch_size), 1), dtype=dtype)
    return theta_trim_arr + u * span


def _paired_prior_pass_metrics(
    drift: PairedPriorDriftNet,
    batch: jax.Array,
    *,
    key: jax.Array,
    delta_v: float,
    ...
    def _interval_metrics(
        ...
        logsnr = bridge_logsnr(theta, delta_v)
        ...
        state_loss = jnp.mean(jnp.sum(jnp.square(state_residual) / jnp.square(denom), axis=-1))
        drift_mse = jnp.mean(jnp.sum(jnp.square(drift_pred - drift_target), axis=-1))
...
def evaluate_paired_prior_bridge_metrics(
    static_model: PairedPriorDriftNet,
    params,
    ...
    *,
    ...
    latent_canonical, _generation_zt = generation_view_from_data(
        ...
        ndim=3,
        shape_spec="(T, N, K)",
        context="the paired prior bridge",
    )
    ...
    batch_key, pass_key = jax.random.split(key)
    batch = select_level_batch(latent_canonical, batch_size, batch_key)
    state_loss, drift_mse, mean_logsnr, mean_prior_time = _paired_prior_pass_metrics(
        drift,
        batch,
        key=pass_key,
        delta_v=delta_v,
    ...
    return {
        ...
        "drift_mse": drift_mse,
        "mean_bridge_logsnr": mean_logsnr,
        "mean_prior_time_match": mean_prior_time,
    }


def train_paired_prior_bridge(
    drift_net: PairedPriorDriftNet,
    latent_data: jax.Array,
    zt: jax.Array,
    delta_v: float,
    prior_logsnr_max: float,
    *,
...
) -> PairedPriorDriftNet | tuple[PairedPriorDriftNet, dict[str, jax.Array]]:
    _validate_bridge_hyperparameters(delta_v, theta_trim, DEFAULT_THETA_FEATURE_CLIP)
    latent_canonical, _generation_zt = generation_view_from_data(
        latent_data,
        zt,
    ...
    def step(
        ...
        key_inner: jax.Array,
        latent_canonical_inner: jax.Array,
    ) -> tuple[PairedPriorDriftNet, optax.OptState, jax.Array, jax.Array, jax.Array, jax.Array]:
    ...
    batch_size_effective = (
        ...
        if batch_size is None else min(int(batch_size), int(latent_canonical.shape[1]))
    )
    ...
    for step_idx in range(num_steps_int):
        ...
        if progress_fn is not None and progress_every is not None and (
            ...
            effective_step_seconds = (
                ...
                if steady_step_times else step_seconds
            )
...
class _BridgeLogSNRDriftWrapper(eqx.Module):
    drift_net: PairedPriorDriftNet
    delta_v: float
    theta_feature_clip: float
    interval_length: float

    def __call__(self, theta: jax.Array | float, y: jax.Array, condition: jax.Array) -> jax.Array:
        logsnr = bridge_logsnr(theta, self.delta_v, theta_clip=self.theta_feature_clip)
        # The learned controller is parameterized in local interval time theta,
...
def _interval_sigma(delta_v: float, tau_start: jax.Array, tau_end: jax.Array, *, dtype: jnp.dtype) -> jax.Array:
    interval_length = jnp.maximum(
        jnp.asarray(tau_end, dtype=dtype) - jnp.asarray(tau_start, dtype=dtype),
        jnp.asarray(1e-12, dtype=dtype),
    )
    return jnp.sqrt(jnp.asarray(delta_v, dtype=dtype) / interval_length)


def sample_paired_prior_conditional_trajectory(
    drift_net: PairedPriorDriftNet,
    ...
    *,
    ...
    validate_bridge_condition_dim(
        drift_net,
        latent_dim=int(coarse_state_arr.shape[-1]),
    ...
    for interval_idx in range(rollout_num_intervals):
        ...
        wrapped_drift = _BridgeLogSNRDriftWrapper(
            drift_net=drift_net,
...
def sample_paired_prior_conditional_batch(
    drift_net: PairedPriorDriftNet,
    ...
    *,
    ...
    keys = jax.random.split(key, int(coarse_batch_arr.shape[0]))
    return jax.vmap(
        lambda coarse_state, subkey: sample_paired_prior_conditional_trajectory(
            drift_net,
            coarse_state,
            zt,
            delta_v,
    ...
    )(coarse_batch_arr, keys)


def sample_paired_prior_conditional_dense_batch_from_keys(
    drift_net: PairedPriorDriftNet,
    coarse_batch: jax.Array,
    zt: jax.Array,
    delta_v: float,
    ...
    *,
    ...
    validate_bridge_condition_dim(
        drift_net,
        latent_dim=int(coarse_batch_arr.shape[-1]),
        num_intervals=condition_num_intervals_int,
        condition_mode="previous_state",
        include_interval_embedding=True,
    )
    ...
    def _sample_one(coarse_state_arr: jax.Array, key: jax.Array) -> tuple[jax.Array, jax.Array]:
        ...
        for interval_idx in range(rollout_num_intervals):
            ...
            interval_path = integrate_conditional_interval(
                wrapped_drift,
                x,
                condition,
                tau_start,
...
__all__ = [
    ...
    "sample_paired_prior_conditional_dense_batch_from_keys",
    "sample_paired_prior_conditional_trajectory",
