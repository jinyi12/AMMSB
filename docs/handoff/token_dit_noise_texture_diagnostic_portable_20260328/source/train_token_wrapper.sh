...
DIT_TIME_EMB_DIM="${DIT_TIME_EMB_DIM:-64}"
DT0="${DT0:-0.01}"
LR="${LR:-1e-4}"
...
transformer_patch16_mkdirs "${OUTDIR}" "${LOG_DIR_TOKEN_DIT}" "$(dirname "${LATENTS_PATH}")" "$(dirname "${MANIFEST_PATH}")"

if [[ -z "${SIGMA}" ]]; then
  if [[ -f "${CALIBRATION_JSON}" ]]; then
    SIGMA="$(transformer_patch16_read_sigma "${CALIBRATION_JSON}")"
  else
    echo "ERROR: set SIGMA explicitly or provide CALIBRATION_JSON from the flat latent calibration." >&2
...
echo "============================================================"
echo "  Token-native transformer CSP training"
