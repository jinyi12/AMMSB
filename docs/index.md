# Docs Index

This directory is the repository system of record. Start with the short map in [../AGENTS.md](../AGENTS.md), then use the links below.

## Start Here

- [runbooks/index.md](runbooks/index.md) for setup, validation, repo health, skills, CSP, and Tran evaluation workflows
- [architecture.md](architecture.md) for the module map and dependency boundaries
- [conventions.md](conventions.md) for FAE framework invariants and latent-MSBM evaluation rules

## Active Workflows

- [evaluation_pipeline.md](evaluation_pipeline.md) for the latent-MSBM and Tran evaluation scripts
- [runbooks/csp.md](runbooks/csp.md) for the active CSP package, bridge-training entrypoints, and validation commands
- [experiments/index.md](experiments/index.md) for experiment catalog and registries
- [publication_figures.md](publication_figures.md) for figure inventory and style conventions
- [publication_latent128_msbm_execution.md](publication_latent128_msbm_execution.md) for the publication pipeline that combines latent geometry and latent MSBM

## Plans And Execution History

- [plans/TEMPLATE.md](plans/TEMPLATE.md)
- [plans/active/](plans/active/)
- [plans/completed/](plans/completed/)

## Harness Surfaces

- [../AGENTS.md](../AGENTS.md)
- [runbooks/bootstrap.md](runbooks/bootstrap.md)
- [runbooks/repo-health.md](runbooks/repo-health.md)
- [runbooks/skills.md](runbooks/skills.md)
- [../Makefile](../Makefile)
- [../scripts/repo_health.py](../scripts/repo_health.py)
- [../scripts/refactor_hotspots.py](../scripts/refactor_hotspots.py)

## Reference Material

- [references/](references/)
- [latent_geometry_formulation.md](latent_geometry_formulation.md)
- [latent_geometry_plotting.md](latent_geometry_plotting.md)
- [page-2026-03-20-18-05-19.md](page-2026-03-20-18-05-19.md)
