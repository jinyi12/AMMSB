...
from .types import LatentInterpolationResult
...
def build_dense_latent_trajectories(
    ...
    if frechet_mode == 'triplet':
        if not compute_triplet:
             ...
             # But if it does, we can fallback or raise
             pass
        U_dense_fm = U_dense_fm_triplet if U_dense_fm_triplet is not None else U_dense_fm_global
    ...
    # Main return value
    # Ensure U_dense_fm is valid (fallback logic above handles basic cases)
    if U_dense_fm is None:
         # Fallback to global if triplet failed or wasn't computed
         if U_dense_fm_global is None:
              deltas_fm_dense_global, U_dense_fm_global, pchip_delta_fm = interpolate_deltas(deltas_fm, U_frechet, method='pchip')
         U_dense_fm = U_dense_fm_global
         _, U_dense_fm_linear, _ = interpolate_deltas(deltas_fm, U_frechet, method='linear')
