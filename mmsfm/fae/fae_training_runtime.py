"""Shared training runtime for active FAE entrypoints."""
...
from mmsfm.fae.sobolev_losses import get_sobolev_h1_loss_fn
from mmsfm.fae.fae_training_components import (
    MSEMetricTimeInvariant,
    evaluate_at_times,
...
def _build_wandb_tags(
    ...
    tags = ["fae", getattr(args, "training_mode", "multi_scale")]
    if hasattr(args, "encoder_type"):
        tags.append(str(args.encoder_type))
...
def run_training_from_datasets(
    args: argparse.Namespace,
    *,
    ...
    if setup_fn is not None:
        ...
        if args.loss_type == "l2":
            from mmsfm.fae.noisy_fae_loss import get_loss_fae_with_noise_fn
            ...
            from mmsfm.fae.ntk_losses import (
                NTKDiagnosticMetric,
    ...
    init_variables = trainer._get_init_variables(init_meta_key)
    architecture_info = resolve_live_architecture_info(
        architecture_info,
        init_variables.get("params", {}),
    )
