...
traj_path = Path(
    os.environ.get(
        "TRAJ_PATH",
        "/data1/jy384/research/MMSFM/results/fae_latent_msbm_var0625/realizations_backward.npz",
    )
)
...
def _parse_args_file(path: Path) -> dict:
    cfg = {}
...
train_cfg = _parse_args_file(args_path)
...
def _extract_reference_fields(
    ...
    for d in time_data_sorted:
        ...
        if int(sample_indices.max()) >= u_all.shape[0]:
            raise IndexError(
                f"sample index {int(sample_indices.max())} exceeds available split size {u_all.shape[0]}"
            )

        # The trajectory bundle stores indices relative to the selected split.
        u_selected = u_all[sample_indices]  # (N, P, 1)
...
if sample_indices is not None:
    ...
    reference_fields = _extract_reference_fields(
        time_data_sorted, ref_idx, resolution
    )
...
if fields_b is not None:
    ...
    if is_realizations and ref_sample is not None:
        for t in range(n_marginals):
            ...
            im = ax.imshow(field, cmap=cmaps.lipari, origin="lower")
            ax.set_xticks([])
            ax.set_yticks([])

            if t == 0:
                ax.set_ylabel("GT", fontsize=9)
            ax.set_title(f"$H={zt[t]:.2f}$", fontsize=9)
    ...
    for i in range(n_real):
        for t in range(n_marginals):
            ...
            if row_offset == 0 and i == 0:
                ax.set_title(f"$H={zt[t]:.2f}$", fontsize=9)
            if t == 0:
                ax.set_ylabel(f"{ylabel_prefix} {i+1}", fontsize=9)
