...
from diffmap.diffusion_maps import (
    fused_symmetric_step_operator,  # type: ignore[attr-defined]
    fractional_step_operator,  # type: ignore[attr-defined]
...
def generate_pseudo_multiscale_data(
    ...
    *,
    ...
    """
    Legacy fused-step pseudo data generator. Requires diffusion_maps fused operators.
    """
    ...
    for idx, (t_val, X_t, A_t) in enumerate(zip(times_arr, X_seq, A_seq)):
        ...
        alphas = [float(a) for a in alphas_per_interval[idx] if (0.0 <= float(a) <= 1.0)]
        alphas = sorted({a for a in alphas if not np.isclose(a, 0.0) and not np.isclose(a, 1.0)})
        if not alphas:
            continue

        A_step = fused_symmetric_step_operator(A_t, A_seq[idx + 1], eta_fuse)  # type: ignore[attr-defined]
        for alpha in alphas:
            A_frac = fractional_step_operator(A_step, alpha)  # type: ignore[attr-defined]
            X_alpha = A_frac @ X_t
