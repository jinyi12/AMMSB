"""Train a transformer-token FAE with an integrated token-native DiT velocity prior."""
...
from mmsfm.fae.transformer_training_support import (
    ...
    build_transformer_prior_parser as build_parser,
    select_transformer_prior_run_metadata,
    validate_transformer_prior_args as validate_args,
)
...
def main() -> None:
    ...
    validate_args(args)
    architecture_name, wandb_name_prefix, wandb_tags = select_transformer_prior_run_metadata()
    run_transformer_training(
        ...
        setup_fn=setup_transformer_prior_training,
    )
