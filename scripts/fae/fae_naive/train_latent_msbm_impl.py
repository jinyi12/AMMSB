"""Implementation for training Multi-marginal Schrödinger Bridge Matching (MSBM) in a pretrained FAE latent space.
...
from scripts.fae.fae_naive.fae_latent_utils import (  # noqa: E402
    ...
    _encode_time_marginals,
    _flat_fields_to_grid,
    _infer_resolution,
    _load_fae_checkpoint,
    _make_fae_apply_fns,
    _reference_field_subset,
)
...
def _write_args_snapshot(outdir: Path, args: argparse.Namespace) -> None:
    ...
    # Keep formatting consistent with scripts/utils.py, but avoid relying on side effects in set_up_exp.
...
def _latent_moment_metrics(x_gen: torch.Tensor, x_ref: torch.Tensor) -> dict:
    ...
    # Use float32 for stable reductions even under AMP.
    xg = x_gen.float()
    xr = x_ref.float()
    ...
    # Optional: covariance Frobenius norm (O(K^2), K is small in practice).
...
def _rbf_mmd2(
    ...
    *,
    ...
    # Median heuristic over combined distances.
    ...
    k_yy = _k(xr, xr)
    k_xy = _k(xg, xr)
    # Unbiased: remove diagonal terms.
    m = int(xg.shape[0])
...
# FAE checkpoint/codec helpers live in `scripts.fae.fae_naive.fae_latent_utils`.
...
def main() -> None:
    ...
    # Latent diagnostics (paired consecutive deltas). This does not drop/merge any marginals.
    ...
    # Freeze var_time_ref into args so downstream calibration/logging sees the actual value.
    if args.var_time_ref is None:
        args.var_time_ref = float(t_ref)

    # Optional: choose diffusion schedule parameters from latent statistics before building the schedule/agent.
    ...
    if args_mutated:
        # Update args snapshots to reflect latent-driven calibration choices.
        try:
    ...
    # Print interval-wise integrated variance (shape of injected noise across time).
    try:
        ...
        # NOTE: MSBM trains the backward policy on *reversed* time labels (to emulate reverse-time dynamics).
        # For time-varying schedules (exp_contract), we therefore apply the time-flipped schedule for the backward
        # policy inside `LatentBridgeSDE` so that diffusion still contracts from fine->coarse in *physical* time.
    ...
    # Latent-space distribution metrics (do not require decoding).
    ...
    def _stage_eval_callback(stage: int, direction: str, _agent: LatentMSBMAgent) -> None:
        ...
        # Rollout sample count: if we need metrics, sample enough latents once and reuse a subset for decoding/plots.
        ...
        # Save/restore RNG so eval does not change subsequent training stochasticity.
        ...
        try:
            ...
            if need_metrics:
                ...
                metrics_rows = []
                # Precompute paired-reference subset (same indices used to init rollouts).
                ...
                # Write per-time metrics to disk (sanitize non-finite values for JSON).
                try:
                    import json
                ...
                # Log summary + per-time metrics to wandb.
                try:
                    if run is not None:
                        ...
                        def _avg(key: str, *, direction: str, interior_only: bool) -> float:
                            ...
                            for r in metrics_rows:
                                t_i = int(r["t_idx"])
                                if interior_only and (t_i == 0 or t_i == (len(metrics_rows) - 1)):
                                    continue
                                vv = float(r[direction].get(key, float("nan")))
                                if _finite(vv):
                                    vals.append(vv)
                        ...
                        # Always log a small, stable summary.
                        ...
                        # Optional per-time logging (kept minimal by default).
    ...
    if n_decode > 0:
        ...
        if args.decode_direction in ("backward", "both"):
            init_b = agent.latent_train[-1, idx_t]  # last marginal
            policy_b = agent.z_b
        ...
        # Optional: end-of-training visualization bundle (even if eval_interval_stages=0).
        if bool(getattr(args, "final_visualize", True)):
            ...
            if n_plot > 0:
                ...
                def _plot_gen(tag: str, fields_flat: np.ndarray) -> None:
                    fields_grid = _flat_fields_to_grid(fields_flat[:, :n_plot], resolution)
                    plot_field_snapshots(
                        fields_grid,
                        zt.tolist(),
                        str(final_dir),
                ...
                # Small bundle for offline inspection.
                try:
