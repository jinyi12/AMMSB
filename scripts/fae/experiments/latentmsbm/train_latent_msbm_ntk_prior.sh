...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --nohup-log)
      ...
      ;;
    -h|--help)
      echo "Usage: bash scripts/fae/experiments/latentmsbm/train_latent_msbm_ntk_prior.sh [--optimizer adam|muon] [--background] [--nohup-log <path>]"
      exit 0
      ;;
    *)
...
case "$OPTIMIZER" in
  adam)
    DEFAULT_CKPT="results/adam_ntk_prior/run_zaql9zhd/checkpoints/state.pkl"
    DEFAULT_RUN_NAME="adam_ntk_prior"
    DEFAULT_OUTDIR="latent_msbm_adam_ntk_prior"
    ;;
  muon)
    DEFAULT_CKPT="results/muon_ntk_prior/run_r6flmspu/checkpoints/state.pkl"
    DEFAULT_RUN_NAME="muon_ntk_prior"
    DEFAULT_OUTDIR="latent_msbm_muon_ntk_prior"
    ;;
  *)
...
WANDB_GROUP="${WANDB_GROUP:-latent_msbm_ntk_prior}"
RUN_NAME="${RUN_NAME:-${DEFAULT_RUN_NAME}}"
OUTDIR="${OUTDIR:-${DEFAULT_OUTDIR}}"
WANDB_TAGS="${WANDB_TAGS:-latent_msbm,ntk_prior,${OPTIMIZER}}"
