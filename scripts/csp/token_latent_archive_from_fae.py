...
"""Build the token-native CSP latent archive from transformer FAE assets."""
...
from scripts.csp.latent_archive import build_marginal_time_dists
...
def build_token_latent_archive_from_fae(
    *,
    dataset_path: Path,
    ...
    if not is_transformer_token_autoencoder(autoencoder):
        raise ValueError(
            "Token-native CSP archives require a transformer-token FAE checkpoint."
        )
    ...
    encode_fn, _decode_fn = make_transformer_fae_apply_fns(
        autoencoder,
        fae_params,
        fae_batch_stats,
        latent_format="tokens",
    )
    latent_train, latent_test, zt, time_indices, split = encode_time_marginals_tokens(
    ...
    transport_info = infer_fae_transport_info(
        fae_meta,
        latent_dim=int(latent_train.shape[-2] * latent_train.shape[-1]),
        token_transport="token_native",
    )
    latent_noise_info = {
        "enabled": False,
        "mode": "none",
        "owner": "scripts/csp/encode_fae_token_latents.py",
    }
...
def write_token_latent_archive_from_fae_manifest(manifest_path: Path, manifest: dict[str, Any]) -> Path:
...
build_token_fae_latent_archive = build_token_latent_archive_from_fae
write_token_fae_latent_archive_manifest = write_token_latent_archive_from_fae_manifest
...
__all__ = [
    ...
    "build_token_fae_latent_archive",
    "write_token_fae_latent_archive_manifest",
