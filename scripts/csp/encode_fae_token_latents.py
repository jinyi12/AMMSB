...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Encode a multiscale transformer FAE dataset into the token-native CSP latent archive."
        ),
    )
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default="results/csp/manual_run/fae_token_latents.npz",
        help="Where to write the token-native latent archive.",
    )
    parser.add_argument(
        "--manifest_path",
        type=str,
...
def main() -> None:
    ...
    print("============================================================", flush=True)
    print("CSP token-native FAE latent archive", flush=True)
